...
class Database:
    ...
    def _create_table_if_not_exists(self, cursor, table_name, columns_sql):
        ...
        # columns_sql = columns_sql.replace("PRIMARY KEY", "").replace("FOREIGN KEY", "")
        # 雙重保險
        create_sql = f"""
            IF NOT EXISTS (
        ...
        try:
            ...
            # 只顯示部分SQL以防過長
            logger.error(
                f"執行的 SQL (部分): CREATE TABLE [dbo].[{table_name}]"
                f" ( {columns_sql[:200]}... )"
            )
            raise
    ...
    def _initialize_db(self):
        ...
        try:
            with self._get_connection() as conn:
                ...
                # 欄位假設: user_id, language, role, is_admin, responsible_area,
                # created_at, display_name, email, last_active
                ...
                self._create_table_if_not_exists(
                    init_cur, "user_preferences", user_preferences_cols
                )

                # 2. equipment (來自 equipment.csv)
                # 欄位假設: eq_id, name, eq_type, location, status, created_at,
                # model_number, purchase_date, last_maintenance_date
                equipment_cols = """
                ...
                """
                self._create_table_if_not_exists(
                    init_cur, "conversations", conversations_cols
                )
                ...
                # 欄位假設: alert_id, eq_id, alert_type_code, timestamp, description,
                # severity, status, resolved_at, notes
                alert_history_cols = """
                ...
                """
                self._create_table_if_not_exists(
                    init_cur, "alert_history", alert_history_cols
                )

                # 6. error_logs (來自 異常紀錄.csv)
                # 欄位假設: log_id, eq_id, timestamp, error_code, description,
                # reporter, status, resolved_at, resolution_notes
                error_logs_cols = """
                ...
                """
                self._create_table_if_not_exists(
                    init_cur, "stats_abnormal_monthly", stats_abnormal_monthly_cols
                )
                ...
                """
                self._create_table_if_not_exists(
                    init_cur, "stats_abnormal_quarterly", stats_abnormal_quarterly_cols
                )
                ...
                """
                self._create_table_if_not_exists(
                    init_cur, "stats_abnormal_yearly", stats_abnormal_yearly_cols
                )
                ...
                """
                self._create_table_if_not_exists(
                    init_cur, "stats_operational_monthly", stats_operational_monthly_cols
                )
                ...
                """
                self._create_table_if_not_exists(
                    init_cur, "stats_operational_yearly", stats_operational_yearly_cols
                )
                ...
                """
                self._create_table_if_not_exists(
                    init_cur, "alert_types", alert_types_cols
                )
    ...
    def get_conversation_history(self, sender_id_param, limit=10):
        """取得指定 sender 的對話記錄"""
        ...
        try:
            with self._get_connection() as conn:
                ...
                messages = [
                    ...
                    for role, content_text in cursor.fetchall()  # content_text 避免與外層 content 參數混淆
                ]
                messages.reverse()
                return messages
        except pyodbc.Error as e:
    ...
    def get_conversation_stats(self):
        ...
        stats = {
            ...
            "user_messages": 0, "assistant_messages": 0, "system_messages": 0,
            "other_messages": 0,
        }
        try:
            with self._get_connection() as conn:
    ...
    def get_recent_conversations(self, limit=20):
        ...
        sql_query = """
            ...
            SELECT TOP (?)
                ...
                rm.language,
                rm.last_activity_ts,
                (SELECT COUNT(*) FROM conversations sub_c
                 WHERE sub_c.sender_id = rm.sender_id) as message_count,
    ...
    def set_user_preference(
        self, user_id, language=None, role=None, is_admin=None,
        responsible_area=None, display_name=None, email=None
    ):
        """設定或更新使用者偏好。is_admin 應為 True/False。"""
        # 假設 user_preferences 表有 user_id, language, role, is_admin,
        # responsible_area, display_name, email, last_active, created_at
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                ...
                # 轉換布林到 BIT 或保留 NULL
                db_is_admin = 1 if is_admin is True else 0 if is_admin is False else None
                ...
                if user_exists:
                    ...
                    if role is not None:
                        ...
                        params.append(role)
                    if db_is_admin is not None:
                        update_fields.append("is_admin = ?")
                        params.append(db_is_admin)
                    if responsible_area is not None:
                        update_fields.append("responsible_area = ?")
                    ...
                    if not update_fields:  # 至少更新 last_active
                        ...
                        sql = f"UPDATE user_preferences SET {', '.join(update_fields)}" \
                              f" WHERE user_id = ?;"
                    ...
                    sql = """
                        INSERT INTO user_preferences
                            (user_id, language, role, is_admin, responsible_area,
                             display_name, email, created_at, last_active)
                ...
                logger.info(
                    ...
                    f" {'updated' if user_exists else 'created'}."
                )
                return True
        except pyodbc.Error as e:
    ...
    def get_user_preference(self, user_id):
        """取得使用者偏好與角色"""
        # 假設 user_preferences 表有 user_id, language, role, is_admin,
        # responsible_area, display_name, email
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                ...
                # 如果未找到，可以選擇是否自動創建預設值
                # logger.info(f"User {user_id} not found in preferences."
                #             " Creating with defaults via set_user_preference.")
