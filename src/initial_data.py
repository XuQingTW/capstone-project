...
def initialize_equipment_data():
    ...
    try:
        with sqlite3.connect(db.db_path) as conn:
            ...
            # 插入監測指標資料
            for equipment_id, metric_type, value, threshold_min, threshold_max, unit in all_metrics:
                cursor.execute("""
            ...
            # 建立使用者訂閱（以管理員為例）
            cursor.execute("SELECT user_id FROM user_preferences WHERE is_admin = 1 LIMIT 1")
