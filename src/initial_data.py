...
def parse_and_transform_threshold_row(row):
    ...
    emergency_min, emergency_max, emergency_op = parse_threshold_string(row.get("重度異常"))
    return (metric_type, normal_value, warning_min, warning_max, critical_min, critical_max, emergency_min, emergency_max, emergency_op)
...
TABLE_CONFIGS = [
    {
        ...
        "sql_columns": ["equipment_id", "name", "eq_type", "location", "status", "last_updated"],
        "transform_row_data": lambda row: (
            row.get('equipment_id'), row.get('name'), row.get('eq_type'), row.get('location'), row.get('status'),
    ...
    },
    {
        ...
        "sql_columns": ["id", "equipment_id", "alert_type", "severity", "message", "is_resolved", "created_at", "resolved_at", "resolved_by", "resolution_notes"],
        "transform_row_data": lambda row: (
            row.get('ID'), row.get('equipment_id'), row.get('alert_type'), row.get('severity'),
    ...
    },
    {
        ...
        "sql_columns": ["id", "equipment_id", "metric_type", "status", "value", "unit", "timestamp"],
        "transform_row_data": lambda row: (
    ...
    },
    {
        ...
        "sql_columns": ["metric_type", "normal_value", "warning_min", "warning_max", "critical_min", "critical_max", "emergency_min", "emergency_max", "emergency_op"],
        "transform_row_data": parse_and_transform_threshold_row
    },
    {
        "excel_sheet_name": "異常紀錄error_log",
        "sql_table_name": "error_logs",
        "sql_columns": ["log_date", "error_id", "equipment_id", "deformation_mm", "rpm", "event_time", "detected_anomaly_type", "downtime_duration", "resolved_at", "resolution_notes"],
        "transform_row_data": lambda row: (
            pd.to_datetime(str(row.get('日期'))) if pd.notna(row.get('日期')) else None,
            str(row.get('error_id')), str(row.get('equipment_id')), row.get('變形量(mm)'), row.get('轉速'),
    ...
    },
    {
        ...
        "sql_columns": ["equipment_id", "year", "month", "total_operation_duration", "total_downtime_duration", "downtime_rate_percent", "description"],
        "transform_row_data": lambda row: (
            str(row.get('equipment_id')), row.get('年'), row.get('月'), str(row.get('總運作時長')),
            str(row.get('停機總時長')), str(row.get('停機率(%)')), str(row.get('說明'))
        )
    },
    {
        "excel_sheet_name": "運作統計(季)",
        "sql_table_name": "stats_operational_quarterly",
        "sql_columns": ["equipment_id", "year", "quarter", "total_operation_duration", "total_downtime_duration", "downtime_rate_percent", "description"],
        "transform_row_data": lambda row: (
            str(row.get('equipment_id')), row.get('年'), row.get('季度'), str(row.get('總運作時長')),
            str(row.get('停機總時長')), str(row.get('停機率(%)')), str(row.get('說明'))
        )
    },
    {
        ...
        "sql_columns": ["equipment_id", "year", "total_operation_duration", "total_downtime_duration", "downtime_rate_percent", "description"],
        "transform_row_data": lambda row: (
            str(row.get('equipment_id')), row.get('年'), str(row.get('總運作時長')),
            str(row.get('停機總時長')), str(row.get('停機率(%)')), str(row.get('說明'))
        )
    },
    {
        ...
        "sql_columns": ["equipment_id", "year", "month", "detected_anomaly_type", "downtime_duration", "downtime_rate_percent", "description"],
        "transform_row_data": lambda row: (
            str(row.get('equipment_id')), row.get('年'), row.get('月'), str(row.get('偵測異常類型')),
            str(row.get('停機時長')), str(row.get('停機率(%)')), str(row.get('說明'))
        )
    },
    {
        "excel_sheet_name": "各異常統計(季)",
        "sql_table_name": "stats_abnormal_quarterly",
        "sql_columns": ["equipment_id", "year", "quarter", "detected_anomaly_type", "downtime_duration", "downtime_rate_percent", "description"],
        "transform_row_data": lambda row: (
            str(row.get('equipment_id')), row.get('年'), row.get('季度'), str(row.get('偵測異常類型')),
            str(row.get('停機時長')), str(row.get('停機率(%)')), str(row.get('說明'))
        )
    },
    {
        ...
        "sql_columns": ["equipment_id", "year", "detected_anomaly_type", "downtime_duration", "downtime_rate_percent", "description"],
        "transform_row_data": lambda row: (
            str(row.get('equipment_id')), row.get('年'), str(row.get('偵測異常類型')),
            str(row.get('停機時長')), str(row.get('停機率(%)')), str(row.get('說明'))
        )
    }
]
                ...
                try:
                    ...
                    if data_to_insert:
                        ...
                        try:
                            ...
                            logger.error(f"批次插入到 '{sql_table_name}' 時發生資料庫錯誤，正在回滾: {e}")
                            conn.rollback()
