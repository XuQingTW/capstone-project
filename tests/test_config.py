...
def test_config_default_values():
    ...
    mock_env = {
        ...
        # OPENAI_API_KEY, LINE_CHANNEL_ACCESS_TOKEN, LINE_CHANNEL_SECRET are not given defaults in Config that would pass validate()
        # So we only test those that have defaults or are not strictly required by validate() for this basic test
    ...
    with patch.dict(os.environ, mock_env, clear=True):
        ...
        # This is tricky. A better way would be for Config to load its values in an __init__ or a load method.
        # For now, let's assume direct os.getenv in class variable assignments means we might need to reload the module
        # or have Config load them on demand.
...
def test_config_env_override(monkeypatch):
    ...
    # A more robust solution involves changing Config to a class that loads on instantiation or via a method.
